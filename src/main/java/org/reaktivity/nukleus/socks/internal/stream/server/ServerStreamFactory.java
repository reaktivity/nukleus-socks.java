...
public final class ServerStreamFactory implements StreamFactory
{
        ...
        final MessageConsumer acceptThrottle)
    {
        ...
        {
            final RouteFW route = streamContext.routeRO.wrap(b, o, l);
            return acceptRef == route.sourceRef() && acceptName.equals(route.source()
                .asString());
        };

        final RouteFW route = streamContext.router.resolve(filter, this::wrapRoute);
        if (route != null)
        {
            final long acceptId = begin.streamId();
            System.out.println("Creating new Server accept Stream");
            return new AcceptStreamHandler(streamContext, acceptThrottle, acceptId, acceptRef)::handleStream;
        }
        return null;
    }
        ...
        final MessageConsumer connectReplyThrottle)
    {
        ...
        return new ConnectReplyStreamHandler(streamContext, connectReplyThrottle, connectReplyId)::handleStream;
    }
