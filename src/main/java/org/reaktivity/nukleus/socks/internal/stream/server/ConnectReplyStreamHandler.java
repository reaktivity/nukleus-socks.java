...
final class ConnectReplyStreamHandler extends DefaultStreamHandler
{
        ...
        int length)
    {
        ...
        case DataFW.TYPE_ID:
            ...
            break;
        case EndFW.TYPE_ID:
            final EndFW end = streamContext.endRO.wrap(buffer, index, index + length);
            handleEnd(end);
            break;
        case AbortFW.TYPE_ID:
            final AbortFW abort = streamContext.abortRO.wrap(buffer, index, index + length);
            handleAbort(abort);
            break;
        default:
            doReset(connectReplyThrottle, connectReplyId);
    ...
    private void handleBegin(BeginFW begin)
    {
        ...
        final Correlation correlation = streamContext.correlations.remove(correlationId);

        if (connectRef == 0L && correlation != null)
        {
            final String acceptReplyName = correlation.acceptName();

            final MessageConsumer newAcceptReply = streamContext.router.supplyTarget(acceptReplyName);
